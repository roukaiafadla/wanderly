(() => {
  ...
  async function renderOverview() {
    main.innerHTML = head('Overview', 'Snapshot of the site.') + `<div class="stats-grid" id="statsGrid"><p class="muted">Loading…</p></div>`;
    try {
  ...
  async function renderDestinations() {
    ...
    try {
      ...
      if (!destinations.length) {
        body.innerHTML = '<tr class="empty-row"><td colspan="8">No destinations yet.</td></tr>';
        return;
      }
  ...
  function openDestModal(dest) {
    ...
    openModal(`
      ...
      <form id="destForm" novalidate>
        ...
        <div class="field" data-field="image"><label>Image path</label><input name="image" placeholder="/images/destinations/example.jpg" value="${escapeHTML(dest?.image || '')}" required><span class="error-msg"></span></div>
  ...
  async function renderTestimonials() {
    main.innerHTML = head('Testimonials', 'Shown in the homepage carousel.') + `
      <div class="toolbar"><div></div><button class="btn btn-gold btn-sm" id="addTestiBtn">+ Add testimonial</button></div>
      <div class="table-wrap"><table>
    ...
    try {
      ...
      if (!testimonials.length) { body.innerHTML = '<tr class="empty-row"><td colspan="5">No testimonials yet.</td></tr>'; return; }
      body.innerHTML = testimonials.map((t) => `
        <tr data-id="${t.id}">
  ...
  function openTestiModal(t) {
    ...
    openModal(`
      ...
      <form id="testiForm" novalidate>
        ...
        <div class="field" data-field="avatar"><label>Avatar path</label><input name="avatar" placeholder="/images/testimonials/example.jpg" value="${escapeHTML(t?.avatar || '')}" required><span class="error-msg"></span></div>
  ...
  async function renderContact() {
    ...
    try {
      ...
      if (!messages.length) { body.innerHTML = '<tr class="empty-row"><td colspan="5">No messages yet.</td></tr>'; return; }
  ...
  async function renderTrips() {
    ...
    try {
      ...
      if (!requests.length) { body.innerHTML = '<tr class="empty-row"><td colspan="8">No trip requests yet.</td></tr>'; return; }
  ...
  async function renderNewsletter() {
    ...
    try {
      ...
      const body = $('#newsBody');
      if (!subscribers.length) { body.innerHTML = '<tr class="empty-row"><td colspan="3">No subscribers yet.</td></tr>'; return; }
